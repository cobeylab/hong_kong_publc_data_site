...
dir <- "http://sph.hku.hk/data/KiddivaxMainV2/"
...
dat3 <-dat3[dat3$GMTR < quantile(dat3$GMTR, 0.80) & !(dat3$hhID %in% swab2$hhID[swab2$Swine.H1=="P"]) ,]
dat4 <- dat2[ !is.na(dat2$mid.season.pH1)  & dat2$intervention=="TIV",]
dat4<- dat4[dat4$GMTR2 < quantile(dat4$GMTR2, 0.80)  &  dat4$GMTR3 < quantile(dat4$GMTR3, 0.80)  & !(dat4$hhID %in% swab2$hhID[swab2$Swine.H1=="P"]) ,]

dat5 <-dat2[is.na(dat2$mid.season.pH1) & dat2$intervention=="placebo",]
dat5 <-dat5[dat5$GMTR < quantile(dat5$GMTR, 0.80) & !(dat5$hhID %in% swab2$hhID[swab2$Swine.H1=="P"]) ,]
dat6 <- dat2[ !is.na(dat2$mid.season.pH1)  & dat2$intervention=="placebo",]
dat6<- dat6[dat6$GMTR2 < quantile(dat6$GMTR2, 0.80)  &  dat6$GMTR3 < quantile(dat6$GMTR3, 0.80)  & !(dat6$hhID %in% swab2$hhID[swab2$Swine.H1=="P"]) ,]


datr.p1 <-rbind(dat3,dat4)
datr.p0 <-rbind(dat5,dat6)

datr2a.p1 <-data.frame(hhID=datr.p1$hhID, TIV=1*(datr.p1$intervention=="TIV"), age=datr.p1$age, caltime=datr.p1$calpostv ,titer=log2(datr.p1$postvax.pH1))
datr2b.p1 <-data.frame(hhID=datr.p1$hhID, TIV=1*(datr.p1$intervention=="TIV"), age=datr.p1$age, caltime=datr.p1$calmid ,titer=log2(datr.p1$mid.season.pH1))
datr2c.p1 <-data.frame(hhID=datr.p1$hhID, TIV=1*(datr.p1$intervention=="TIV"), age=datr.p1$age, caltime=datr.p1$calposts ,titer=log2(datr.p1$post.season.pH1))

datr2a.p0 <-data.frame(hhID=datr.p0$hhID, TIV=1*(datr.p0$intervention=="TIV"), age=datr.p0$age, caltime=datr.p0$calpostv ,titer=log2(datr.p0$postvax.pH1))
datr2b.p0 <-data.frame(hhID=datr.p0$hhID, TIV=1*(datr.p0$intervention=="TIV"), age=datr.p0$age, caltime=datr.p0$calmid ,titer=log2(datr.p0$mid.season.pH1))
datr2c.p0 <-data.frame(hhID=datr.p0$hhID, TIV=1*(datr.p0$intervention=="TIV"), age=datr.p0$age, caltime=datr.p0$calposts ,titer=log2(datr.p0$post.season.pH1))
...
require(gee)
model.p1 <-gee(titer~caltime,data=datr2.p1,family=gaussian, id=hhID)
model.p0 <-gee(titer~caltime,data=datr2.p0,family=gaussian, id=hhID)

model.p1.age <-gee(titer~caltime+(age<9)+caltime*(age<9),data=datr2.p1,family=gaussian, id=hhID)
model.p0.age <-gee(titer~caltime+(age<9)+caltime*(age<9),data=datr2.p0,family=gaussian, id=hhID)

model.pi.m <- gee(titer~caltime+TIV+caltime*TIV,data=rbind(datr2.p1, datr2.p0),family=gaussian, id=hhID)
...
dat3 <-dat3[dat3$GMTR < quantile(dat3$GMTR, 0.85) & !(dat3$hhID %in% swab2$hhID[swab2$FluB=="P"]) ,]
dat4 <- dat2[ !is.na(dat2$mid.season.B.Brisbane)  & dat2$intervention=="TIV",]
dat4<- dat4[dat4$GMTR2 < quantile(dat4$GMTR2, 0.85)  &  dat4$GMTR3 < quantile(dat4$GMTR3, 0.85)  & !(dat4$hhID %in% swab2$hhID[swab2$FluB=="P"]) ,]

dat5 <-dat2[is.na(dat2$mid.season.B.Brisbane) & dat2$intervention=="placebo",]
dat5 <-dat5[dat5$GMTR < quantile(dat5$GMTR, 0.85) & !(dat5$hhID %in% swab2$hhID[swab2$FluB=="P"]) ,]
dat6 <- dat2[ !is.na(dat2$mid.season.B.Brisbane)  & dat2$intervention=="placebo",]
dat6<- dat6[dat6$GMTR2 < quantile(dat6$GMTR2, 0.85)  &  dat6$GMTR3 < quantile(dat6$GMTR3, 0.85)  & !(dat6$hhID %in% swab2$hhID[swab2$FluB=="P"]) ,]
...
require(gee)
model.b1 <-gee(titer~caltime,data=datr2.b1,family=gaussian, id=hhID)
model.b0 <-gee(titer~caltime,data=datr2.b0,family=gaussian, id=hhID)

model.b1.age <-gee(titer~caltime+(age<9)+caltime*(age<9),data=datr2.b1,family=gaussian, id=hhID)
model.b0.age <-gee(titer~caltime+(age<9)+caltime*(age<9),data=datr2.b0,family=gaussian, id=hhID)

model.bi.m <- gee(titer~caltime+TIV+caltime*TIV,data=rbind(datr2.b1, datr2.b0),family=gaussian, id=hhID)

n.m <- c(nrow(datr.p1), nrow(datr.p0), nrow(datr.b1[datr.b1$age<9,]), nrow(datr.b1[datr.b1$age>=9,]), nrow(datr.b0))
